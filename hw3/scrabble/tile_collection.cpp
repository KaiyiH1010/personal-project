...
size_t TileCollection::count_tiles() const {
  size_t count{0};
  for (TileMap::const_iterator it = this->tiles.cbegin();
       it != this->tiles.cend(); ++it) {
    count += it->second;
  }
  return count;
}
...
unsigned int TileCollection::total_points() const {
  unsigned int sum = 0;
  for (TileMap::const_iterator map_itr = tiles.begin(); map_itr != tiles.end();
       map_itr++) {
...
TileCollection::const_iterator::self_type TileCollection::const_iterator::
operator++() {
  repeat_count++;
