...
void print_horizontal(size_t columns, const char *left, const char *joint,
                      const char *right, ostream &out) {
  if (columns == 0) {
    return;
  }
  ...
  out << repeat(I_HORIZONTAL, SQUARE_INNER_WIDTH) << right
      << BG_COLOR_OUTSIDE_BOARD;
}
